...
test.describe('Shopping Cart fixture Testing', () => {
...
//        await productPage.verifyVisibility('productTd_inventoryContainer');
    ...
    test('fixture Add Items to Cart', async ({ productPage, cartPage, page }, testInfo) => {
...
//    await expect(cartCount).toEqual(retryCount ? 3 : 2); // handling flaky
    ...
    test('fixture Remove Items from Cart', async ({ productPage, page }) => {
    ...
    test('fixture Cart Persistence', async ({ productPage, page }) => {
        // Add items to cart
        await productPage.clickElement('productTd_cartBagPack');
        await productPage.clickElement('productTd_cartBikeLight');
